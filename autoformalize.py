...
def autoformalize(sample: dict, model: str = "gpt-4o-mini") -> str:
    ...
    # Hardcoded example from the dataset
    example_informal = (
    ...
    messages = [
        {"role": "system", "content": "You are an expert in formalizing mathematical statements into Lean 4 statements. You will be given an informal statement and you will need to formalize it into a Lean 4 statement. You only need to formalize the statement, not the proof."},
        {"role": "user", "content": f"""
    ...
    logging.info(f"Ground truth formalized statement: {sample['formal_statement']}")
    return formalized 